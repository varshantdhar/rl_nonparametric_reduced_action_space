...
class MCMCPosterior(object):
        ...
        if t_a.sum() == 1:
            ...
            if k_new == self.reward_posterior["K"][a]:
                ...
                if k_new == N_ak.size:
                    ...
                    self.reward_posterior["alpha"] = np.concatenate(
                        (
                            self.reward_posterior["alpha"],
                            self.reward_prior["alpha"][:, None]
                            * np.ones((1, self.reward_posterior["K"][a])),
                        ),
                        axis=1,
                    )
                    self.reward_posterior["beta"] = np.concatenate(
                        (
                            self.reward_posterior["beta"],
                            self.reward_prior["beta"][:, None]
                            * np.ones((1, self.reward_posterior["K"][a])),
                        ),
                        axis=1,
                    )
                    self.reward_posterior["theta"] = np.concatenate(
                        (
                            self.reward_posterior["theta"],
                            self.reward_prior["theta"][:, None, :]
                            * np.ones((1, self.reward_posterior["K"][a], 1)),
                        ),
                        axis=1,
                    )
                    self.reward_posterior["Sigma"] = np.concatenate(
                        (
                            self.reward_posterior["Sigma"],
                            self.reward_prior["Sigma"][:, None, :, :]
                            * np.ones((1, self.reward_posterior["K"][a], 1, 1)),
                        ),
                        axis=1,
                    )
            ...
            n_iter = 1
            (XcondZ_loglik[n_iter], Z_loglik[n_iter]) = self.compute_loglikelihood(
                a, z_a, N_ak, x_a, y_a
            )
            print(
                "t={}, n_iter={}, {} observations for arm {} with loglikelihood={}".format(
                    t, n_iter, t_a.sum(), a, XcondZ_loglik[n_iter] + Z_loglik[n_iter]
                )
            )

            # Iterate while not converged or not max iterations
            while n_iter < self.reward_prior["gibbs_max_iter"] and abs(
                (XcondZ_loglik[n_iter] + Z_loglik[n_iter])
                - (XcondZ_loglik[n_iter - 1] + Z_loglik[n_iter - 1])
            ) >= (
                self.reward_prior["gibbs_loglik_eps"]
                * abs((XcondZ_loglik[n_iter - 1] + Z_loglik[n_iter - 1]))
            ):
                n_iter += 1
                ...
                for n in np.random.permutation(t_a.sum()):
                    ...
                    N_ak[k_old] -= 1
                    self.update_reward_posterior_params(
                        "del", a, k_old, x_a[:, n], y_a[n]
                    )
                    ...
                    if N_ak[k_old] == 0:
                        ...
                        N_ak[k_old : self.reward_posterior["K"][a] - 1] = N_ak[
                            k_old + 1 : self.reward_posterior["K"][a]
                        ]
                        N_ak[self.reward_posterior["K"][a] - 1 :] = 0
                        # Empty mixture posteriors
                        self.reward_posterior["alpha"][
                            a, k_old : self.reward_posterior["K"][a] - 1
                        ] = self.reward_posterior["alpha"][
                            a, k_old + 1 : self.reward_posterior["K"][a]
                        ]
                        self.reward_posterior["alpha"][
                            a, self.reward_posterior["K"][a] - 1 :
            ...
            if self.reward_prior["gibbs_plot_save"] is not None:
                ...
                if self.reward_prior["gibbs_plot_save"] == "show":
                    ...
                    plt.savefig(
                        self.reward_prior["gibbs_plot_save"]
                        + "/{}_loglik_t{}.pdf".format(self.__class__.__name__, t),
                        format="pdf",
                        bbox_inches="tight",
                    )
                plt.close()
    ...
    def update_reward_posterior_params(self, how, a, k, x, y):
        ...
        if how == "add":
            ...
            # Update beta
            self.reward_posterior["beta"][a, k] += np.power(
                y - np.einsum("d,d->", x, self.reward_posterior["theta"][a, k]), 2
            ) / (
                2
                * (
                    1
                    + np.einsum("d,da,a->", x, self.reward_posterior["Sigma"][a, k], x)
                )
            )
            # Sigma inverse
            sigma_inv = np.linalg.inv(self.reward_posterior["Sigma"][a, k])
            # Update regressor covariance (V)
            self.reward_posterior["Sigma"][a, k] = np.linalg.inv(
                sigma_inv + x[:, None] * x[None, :]
            )
            # Update regressors (u)
            self.reward_posterior["theta"][a, k] = np.einsum(
                "ab,b->a",
                self.reward_posterior["Sigma"][a, k],
                (
                    x * y
                    + np.einsum(
                        "ab,b->a", sigma_inv, self.reward_posterior["theta"][a, k]
                    )
                ),
            )
        ...
        assert np.all(self.reward_posterior["alpha"][a] > 0.0)
        assert np.all(self.reward_posterior["beta"][a] > 0.0)
        # Positive definite Sigma
        assert np.all(np.linalg.eigvals(self.reward_posterior["Sigma"][a]) > 0.0)
    ...
    def compute_ylikelihood_per_mixture(self, a, x, y):
        ...
        m_y = np.einsum(
            "d,kd->k",
            x,
            self.reward_posterior["theta"][a, : self.reward_posterior["K"][a]],
        )
        r_y = (
            self.reward_posterior["beta"][a, : self.reward_posterior["K"][a]]
            / self.reward_posterior["alpha"][a, : self.reward_posterior["K"][a]]
            * (
                1
                + np.einsum(
                    "d,kdd,d->k",
                    x,
                    self.reward_posterior["Sigma"][a, : self.reward_posterior["K"][a]],
                    x,
                )
            )
        )
        ...
        assert not np.any(
            np.isnan(f_y_k)
        ), "Nan in ylikelihood_per_mixture f_y_k={}".format(f_y_k)
        return f_y_k
    ...
    def compute_ylikelihood_for_new_mixture(self, a, x, y):
        ...
        r_y = (
            self.reward_prior["beta"][a]
            / self.reward_prior["alpha"][a]
            * (1 + np.einsum("d,dd,d->", x, self.reward_prior["Sigma"][a], x))
        )
        # Likelihood of predictive distribution
        ...
        assert not np.any(
            np.isnan(f_y_newk)
        ), "Nan in likelihood_for_new_mixture f_y_newk={}".format(f_y_newk)
        return f_y_newk
    ...
    def __compute_loglikelihood_XcondZ(self, a, z_a, N_ak, x_a, y_a):
        ...
        if (
            self.reward_prior["type"] == "linear_gaussian_mixture"
            and self.reward_prior["dist"] == "NIG"
        ):
            # Sufficient statistics of posterior
            ...
            for k in np.arange(self.reward_posterior["K"][a]):
                ...
                # Suff stats
                M_Y = np.einsum(
                    "dn,d->n", x_a[:, k_idx], self.reward_posterior["theta"][a, k]
                )
                Psi_Y = np.eye(n_Y) + np.einsum(
                    "an,ab,bt->nt",
                    x_a[:, k_idx],
                    self.reward_posterior["Sigma"][a, k],
                    x_a[:, k_idx],
                ...
                XcondZ_loglik += (
                    special.gammaln((nu_Y[k] + n_Y) / 2)
                    - special.gammaln((nu_Y[k]) / 2)
                    - n_Y / 2 * (np.log(np.pi) + np.log(Omega_Y[k]))
                    - 1 / 2 * np.log(np.linalg.det(Psi_Y))
                    - ((nu_Y[k] + n_Y) / 2) * np.log(np.linalg.det(tmp))
                )
            ...
            raise ValueError(
                "reward_prior type {} not implemented yet".format(
                    self.reward_prior["type"]
                )
            )
