...
class MCMCBanditSampling(BanditSampling, MCMCPosterior):
    ...
    def compute_arm_predictive_density(self, t):
        ...
        for a in np.arange(self.A):
            ...
            for k in np.arange(K_a):
                ...
                sigma_samples = stats.invgamma.rvs(
                    self.reward_posterior["alpha"][a, k],
                    scale=self.reward_posterior["beta"][a, k],
                    size=(1, 1),
                )
            ...
            rewards_expected_samples[a, :] = np.einsum(
                "k,km->m", pi, rewards_expected_per_mixture_samples
            )
        ...
        self.arm_predictive_density["mean"][:, t] = (
            (
                rewards_expected_samples.argmax(axis=0)[None, :]
                == np.arange(self.A)[:, None]
            ).astype(int)
        ).mean(axis=1)
        # Variance of times expected reward is maximum
        self.arm_predictive_density["var"][:, t] = (
            (
                rewards_expected_samples.argmax(axis=0)[None, :]
                == np.arange(self.A)[:, None]
            ).astype(int)
