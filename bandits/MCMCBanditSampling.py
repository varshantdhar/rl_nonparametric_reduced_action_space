...
class MCMCBanditSampling(BanditSampling, MCMCPosterior):
    ...
    def compute_arm_predictive_density(self, t):
        ...
        """
        rewards_expected_samples=np.zeros((self.A, 1))
        ...
        for a in np.arange(self.A):
            ...
            for k in np.arange(K_a):
                ...
                sigma_samples=stats.invgamma.rvs(self.reward_posterior['alpha'][a,k], scale=self.reward_posterior['beta'][a,k], size=(1,1))
                # Then multivariate Gaussian parameters
                theta_samples=self.reward_posterior['theta'][a,k,:][:,None]+np.sqrt(sigma_samples)*(stats.multivariate_normal.rvs(cov=self.reward_posterior['Sigma'][a,k,:,:], size=1).reshape(1,self.d_context).T)
            ...
            if self.reward_prior['K'] == 'nonparametric':
                ...
                # First sample variance from inverse gamma for each mixture
                sigma_samples=stats.invgamma.rvs(self.reward_prior['alpha'][a], scale=self.reward_prior['beta'][a], size=(1,1))
                # Then multivariate Gaussian parameters
                theta_samples=self.reward_prior['theta'][a,:][:,None]+np.sqrt(sigma_samples)*(stats.multivariate_normal.rvs(cov=self.reward_prior['Sigma'][a,:,:], size=1).reshape(1,self.d_context).T)
                # Compute expected reward per mixture, linearly combining context and sampled parameters
